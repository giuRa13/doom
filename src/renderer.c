...
void renderer_draw_point(vec2_t point, float size, vec4_t color)
{
    ...
    mat4_t model = mat4_mult(scale, translation);
...
void renderer_draw_quad(vec2_t center, vec2_t size, float angle, vec4_t color)
{
    ...
    mat4_t model = mat4_mult(mat4_mult(scale, rotation), translation);
...
void renderer_draw_line(vec2_t p0, vec2_t p1, float width, vec4_t color)
{
    ...
    // ( midpoint formula for the actul size)
    mat4_t translation = mat4_translate((vec3_t) { (p0.x + p1.x) / 2.f, (p0.y + p1.y) / 2.f, 0.f });
